...
fn handle_decrypt(request: &Value) -> Result<String, String> {
    ...
    for blob in blobs {
        ...
        .and_then(|key| {
            ...
            let encrypted_data =
                serde_json::from_str::<Vec<u8>>(blob["encrypted data"].as_str().unwrap()).unwrap();
            aes256_cbc::decrypt(&encrypted_data, key.as_slice(), &iv)
                .map_err(|_| "decryption failed".to_string())
                .and_then(|decrypted_data| Ok(String::from_utf8(decrypted_data).unwrap()))
        }) {
            Ok(decrypted_data) => response.insert(
                blob["encrypted data"].to_string(),
                Value::String(decrypted_data),
            ),
            Err(e) => return Err(e),
        };
    }
...
fn handle_getKek(request: &Value) -> Result<String, String> {
    ...
    for index in 0..blobs.len() {
        ...
        match key_manager::directory_key_manager::get_key(&String::from(kid))
            ...
            .and_then(|key| Ok(key))
        {
            Ok(key) => response.insert(
                String::from(kid),
                Value::String(serde_json::to_string(&key).unwrap()),
            ),
            Err(e) => return Err(e),
        };
    }
