...
func (p *setnsProcess) start() (err error) {
	...
	ierr := parseSync(p.messageSockPair.parent, func(sync *syncT) error {
		...
		case procEnclaveConfigReq:
			if p.config.EnclaveConfig == nil {
				return newSystemErrorWithCause(nil, "received syncT 'EnclaveConfigReq'")
			}
			config := &enclaveConfigs.InitEnclaveConfig{
				Type:                  p.config.EnclaveConfig.Enclave.Type,
				Path:                  p.config.EnclaveConfig.Enclave.Path,
				Args:                  p.config.EnclaveConfig.Enclave.Args,
				IsProductEnclave:      p.config.EnclaveConfig.Enclave.IsProductEnclave,
				RaType:                p.config.EnclaveConfig.Enclave.RaType,
				RaEpidSpid:            p.config.EnclaveConfig.Enclave.RaEpidSpid,
				RaEpidSubscriptionKey: p.config.EnclaveConfig.Enclave.RaEpidSubscriptionKey,
				RaEpidIsLinkable:      p.config.EnclaveConfig.Enclave.RaEpidIsLinkable,
			}
