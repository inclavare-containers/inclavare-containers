...
type EnclaveRuntime interface {
	...
	Attest(bool, string, string, uint32, uint32) ([]byte, error)
...
func (rt *EnclaveRuntimeWrapper) LaunchAttestation(isRA bool, spid string, subscriptionKey string, product uint32, quoteType uint32) ([]byte, error) {
	logrus.Debugf("attesting enclave runtime")

	return rt.runtime.Attest(isRA, spid, subscriptionKey, product, quoteType)
}
