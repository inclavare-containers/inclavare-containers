...
#[derive(Clone)]
struct EnclaveTls(NonNull<enclave_tls_handle>);
...
impl EnclaveTls {
    ...
    fn negotiate(&self, fd: RawFd) -> Result<(), enclave_tls_err_t> {
    ...
    fn receive(&self, buf: &mut [u8]) -> Result<usize, enclave_tls_err_t> {
    ...
    fn transmit(&self, buf: &[u8]) -> Result<usize, enclave_tls_err_t> {
        let mut len: size_t = buf.len() as size_t;
        let err = unsafe {
            enclave_tls_transmit(self.as_ptr(),
                        buf.as_ptr() as *const c_void, &mut len)
        };
        if err == ENCLAVE_TLS_ERR_NONE {
            Ok(len as usize)
        } else {
