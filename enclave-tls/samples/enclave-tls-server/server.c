                ...
                bool mutual, bool debug_enclave, char *ip, int port)
{
        ...
        int ret = 0;
        int sgx_status = ecall_etls_server_startup((sgx_enclave_id_t)enclave_id, &ret, (sgx_enclave_id_t)enclave_id,
                        log_level, attester_type,
                        verifier_type, tls_type,
                        crypto_type, flags,
                        s_ip, s_port);
        if (sgx_status != SGX_SUCCESS || ret) {
                printf("failed to startup enclave server: sgx status %d, ecall return %d\n", sgx_status, ret);
                return -1;
        }
